...
# def login_():
    ...
    # if not session.get("user_id") and not session.get("session_id"):s = cur.execute("SELECT * FROM preusers ORDER BY session_id DESC")
    #     if session_ids == None:
    #         session_id = 1
    #     else:
...
def dblist():
    ...
    # getの場合
        ...
        # データベースを取得
        #try:
        user_id = 1#session.get("user_id") # ログイン時
        databases = cur.execute("SELECT * FROM databases WHERE user_id = %s;", (int(user_id),))
        #except:
            # try:
            #     id = session.get("session_id") # 未ログイン時
            #     databases = cur.execute("SELECT * FROM databases WHERE session_id = %s;", (id))
            # except: # エラー時
        ...
        if table1_id: # テーブル1が選択されている時
            try:
                ...
                flash("エラーが発生しました", "failed")
                return redirect("/")
            table1 = cur.execute("SELECT * FROM (%s);", table1_name)
        ...
        if table2_id: # テーブル2が選択されている時
            try:
                ...
                flash("エラーが発生しました", "failed")
                return redirect("/")
            table2 = cur.execute("SELECT * FROM (%s);", table2_name)
        ...
        if table3_id: # テーブル3が選択されている時
            try:
                ...
                flash("エラーが発生しました", "failed")
                return redirect("/")
            table3 = cur.execute("SELECT * FROM %(s);", table3_name)
